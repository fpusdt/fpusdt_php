...
class Tron implements TronInterface
{
    ...
    */
     ...
     * @throws TronException
     */
    public function sendTransactionMessage(string $to, float $amount, string $message= null, string $from = null): array
    {
        if (is_null($from)) {
            $from = $this->address['hex'];
        }

        $transaction = $this->transactionBuilder->sendTrxMessage($to, $amount, $from,$message);
    ...
    public function sendTrxMessage(...$args): array {
        return $this->sendTransactionMessage(...$args);
    }
